...
\begin{cases}
h_{(t)}^T y_{(i)} &\text{점곱}\\
h_{(t)}^T W y_{(i)} &\text{일반 점곱}\\
v^T \tanh (W[h_{(t)} ; y_{(i)}]) &\text{연결}\\
\end{cases}
\end{align}
$$

연결 어텐션은 인코더의 출력과 디코더의 이전 은닉상태를 연결한다.

점곱은 하나의 점수를 만들고 이 점수가 모두 소프트맥스 층을 통과해 최종 가중치를 만든다.

그리고 점곱 계산 전에 먼저 편향 없는 TimeDistributed가 적용된 Dense층인 선형 변환 층을 통과하는 일반 점곱 방법은 연결 어텐션보다 높은 성능이 나온다.

 - 비주얼 어텐션

   어텐션 메커니즘을 활용해 비주얼 어텐션으로 이미지 캡션을 생성할 수 있다.

   합성곱 신경망이 이미지를 처리하여 일련의 특성 맵을 출력하고 어텐션 메커니즘을 장착한 RNN이 한 번에 한 단어씩 캡션을 생성한다.

   그리고 디코더는 어텐션 모델을 사용해 단어가 출력될 때 이미지에서 어떤 부위에 초점을 맞추고 있는지도 확인할 수 있다.
...
트렌스포머의 인코더에서는 단어 ID의 시뭔스로 표현된 문장의 배치를 입력으로 받는다.

그리고 디코더 에서는 훈련하는 동안 타깃 문장을 입력으로 받는데 이는 시작 부분에 SOS토큰이 추가되어 한 타임 스텝씩 오른쪽으로 이동된 값이다.

추론과정에서 디코더에 타깃을 주입할 수 없어 이전 타임 스텝에서 출력된 단어를 주입하는데 이때 모델이 반복적으로 실행되어 매번 하나의 단어를 예측한다.

한 번에 한 단어씩 보면서 문장을 번역하기 위해서는 기존과 다른 새로운 구성요소가 필요하다.
...
-	위치 인코딩
 	...
 	i번째 위치 인코딩이 문장에 있는 i번째 단어 임베딩에 더해지는데 이 과정에서 고유한 위치 인코딩이 만들어져 각 단어의 위치를 알 수 있다.

  멀티-헤드 어텐션 층에서 단어 사이 관계만 보고 단어의 순서와 위치를 고려하지 않기 때문에 필수적인 요소이다.

-	멀티-헤드 어텐션

 	관련이 많은 단어에 더 많은 주의를 기울이면서 각 단어와 동일한 문장에 있는 다른 단어의 관계를 인코딩한다.

 	멀티-헤드 어텐션을 설명하기 위해서는 먼저 기본이 되는 스케일드 점-곱 어텐션을 이해해야한다.

    - 점-곱 어텐션

      인코더가 문장을 분석하여 특정 단어가 주어인지 동사인지 이해 하려고 한다면 딕셔너리를 만들고, 디코더가 key에 해당하는 value를 찾는 과정이 필요하다.
      
      하지만 모델에는 키를 표현하기 위한 구분된 토큰이 없어 벡터 표현을 사용하여 이 개념을 갖는다.

      따라서 룩업에 사용할 키는 딕셔너리의 키와 완벽하게 매칭되지 않는다.
      
      그래서 쿼리와 딕셔너리에 있는 각 키 사이의 유사도를 계산한 다음 소프트 맥스 함수를 사용해 유사도 점수를 합해 1이 되는 가중치로 변환한다.
      
      그 다음 모델이 키에 해당하는 값의 가중치 합을 계산한다.
      
      스케일드 점-곱 어텐션 : $\text{Attention}(Q, K, V) = softmax \left( \dfrac{QK^T}{\sqrt{d_{keys}}} \right)$
      
      Q: 행마다 쿼리 하나를 담은 행렬로 디코더가 생성한 단어 인코딩의 리스트이다.

      K: 행마다 key 하나를 담은 행렬

      V: 행마다 value 하나를 담은 행렬

      $QK^T$: 쿼리/키 쌍마다 하나의 유사도를 점수로 담고 있는 행렬로 하나의 행은 하나의 쿼리에 해당하고 각 행은 value의 가중치 합을 나타낸다.

 	멀티 헤드 어텐션은 이런 스케일 점 곱 어텐션 층의 묶음이다.

 	각 층은 값, 키, 쿼리의 선형변환이 선행되고, 출력은 단순히 모두 연결되어있어 마지막 선현 병환을 통과한다.

 	만약 멀티 헤드 어텐션을 사용하지 않으면 점-곱 어텐션으로 단어의 위치, 과거형 등과 같은 많은 특성을 모두 쿼리해야하는 문제가 있다.

 	멀티 헤드 어텐션을 사용하면 여러 부분공간으로 다양하게 투영하여 더 효과적으로 단어를 표현할 수 있다.
