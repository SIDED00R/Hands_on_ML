...
-	더블 DQN
 	...
 	DQN의 Q-가치 추정에서 과대 적합이 발생한다는 문제가 있다.

 	이러한 문제를 개선하기 위해 다음 상태에서 최선의 행동을 선택할 때 타깃 모델 대신 온라인 모델을 사용하도록한다.

-	우선순위 기반 경험 재생

 	재생 버퍼에서 경험을 균일하게 샘플링하는 것이 아니라 중요한 경험을 더 자주 샘플링하는 방식이다

 	즉, 학습 진행을 빠르게 만드는 경험을 더 중요한것으로 간주한다.

  중요한 것을 판별하는 기준은 TD 오차 ( $\delta = r + \gamma . V(s') - V(s)$ )를 사용하는 것이다.

  다만 샘플이 중요한 경험에 편향되어 있으므로 훈련하는 동안 중요도에 따라 경험의 가중치를 낮춰 편향을 보상해줘야한다.  
 	
-	듀얼링 DQN (DDQN)

 	상태-행동 쌍의 Q-가치가 $Q(s, a) = V(s) + A(s, a)$처럼 표현될 수 있는데 여기서 V(s)는 상태 s의 가치이고 A(s, a)는 상태 s에서 다른 모든 가능한 행동과 비교하여 선택했을 때의 이득이다.
 	
 	이렇게 상태 가치와 상태-행동 가치로 분리하면, 에이전트의 행동 가치를 추정할 때 어떤 상태에서의 가치와 상태-행동 쌍의 상대적 가치를 모두 고려할 수 있다는 장점이 있다.

### 이 밖의 유명한 강화 학습 알고리즘

 - 액터 크리틱 알고리즘

   정책 그레디언트와 심층 Q-네트워크를 결합한 강화 학습 알고리즘으로 정책 네트워크와 DQN2개를 포함한다.

   이 알고리즘은 크리틱과 액터라는 두 가지 주요 구성 요소로 구성된다.

    - 액터

      액터는 에이전트가 환경에서 어떻게 행동해야 하는지를 결정하는 정책을 학습하는 부분이다.

      액터의 목표는 최적 정책을 학습하여 최대 보상을 얻도록 에이전트의 행동을 조절한다.

     - 크리틱

       크리틱은 에이전트의 행동에 대한 가치 함수를 평가하고, 얼마나 좋은 행동을 했는지를 측정하는 역할을 한다.

       크리틱은 액터의 행동을 평가하고 행동 가치를 제공하여 액터가 행동을 개선할 때 정보를 제공한다.

-	A3C

 	비동기적인 일정한 간격으로 각 에이전트가 마스터 네트워크로 가중치 업데이트를 보내고, 네트워크에서 최신의 가중치를 받아온다.

 	이때 각 에이전트는 마스터 네크워크의 향상에 기여하면서 다른 에이전트가 학습한 것에서 혜택을 받는다.
...
-	SAC
 	...
 	즉, 가능한 많은 보상을 얻으려 하지만 가능한 예측이 어렵도록 만들어 DQN이 불완전한 추정을 만드는 행동을 반복해 실행하지 않도록 돕는다.
...
-	호기심 기반 탐색
 	...
 	따라서 에이전트가 예측 가능해지면 다른 곳으로 이동하고 이 또한 예측 가능해지면 다른곳으로 이동하는 과정을 반복한다.

 	게임의 경우 이겼을 때 보상은 없지만 졌을 때 게임이 다시 시작되는 상황이 예측 가능함으로 이를 피하기 위해 학습을 한다.
